...
app.post('/nearby', function(req, res) {
  ...
  Activity.findOne({ uid: req.body.uid }).then(function(data) {
    ...
    nArray.push(req.body.uid);
    if (data != null) {
      nArray = nArray.concat(data.blocked, data.liked, data.disliked)
    }

      console.log("req.body.uid ", req.body.uid);
      console.log("blocked ", data.blocked);
      console.log("liked ", data.liked);
      console.log("disliked ", data.disliked);
      console.log("nArray ", nArray);
  ...
  searchObj = {
    ...
    loc: {
      $near: {
        ...
        $geometry: {
          type: "Point",
          coordinates: [28.72604813254923, 77.2196788349934]
        }
      }
    }
  }
  ...
  User.find(searchObj).limit(req.body.limit).sort('-createdAt').select('name imageCount city country gender dob loc uid photos image -_id createdAt').exec(function(err, locations) {
    if (err) {
      return res.json(err);
    }
    res.json(locations);
...
// request('https://geocode.xyz/'+newObj.point.geopoint.latitude+','+newObj.point.geopoint.longitude+'?geoit=json', function (error, response, body) {
//   console.error('error:', error); // Print the error if one occurred
//   console.log('statusCode:', response && response.statusCode); // Print the response status code if a response was received
//   console.log('body:', .city, JSON.parse(body).country); // Print the HTML for the Google homepage.
